...
export default function Header({
      ...
      if (userId) {
        ...
        if (res.ok) {
          ...
          setUserData(
            userRole === "admin"
              ? data.customer
              : data.details.find((user) => user._id === id)
          );
        }
      }
    } catch (error) {
  ...
  const notificationCount = async () => {
    try {
      const res = await fetch(
        `${API_URL}/api/countUnseenNotifications/${userId}`
      );
      const data = await res.json();
  ...
  const markNotificationSeen = async (tableId, orderHistoryId) => {
    try {
      await fetch(
        `${API_URL}/api/markNotificationSeen/${userId}/${tableId}/${orderHistoryId}`,
        {
          method: "PUT",
              ...
              if (order.OrderHistory && order.OrderHistory.length > 0) {
                ...
                sortedHistory.forEach((history, index) => {
                  formattedNotifications.push({
                    ...
                    tableName,
                    messageKey:
                      index === 0 ? "newOrderPlaced" : "addedOrderPlaced",
                    messageParams: { title: tableName },
                    timestamp: history.orderDate,
                    type: index === 0 ? "newOrder" : "addedOrder",
                    orderHistoryId: history._id,
    ...
    <div>
      ...
      {language && (
        <div className="absolute bg-gray-800 right-10 w-1/2 md:w-1/3 lg:w-1/4 top-12 border border-gray-500 rounded-2xl z-50">
          <ul>
            <li
              className={`px-4 py-2 hover:bg-gray-700 rounded-t-2xl cursor-pointer ${
                selectedLang === "en" ? "bg-gray-700 font-bold" : ""
              }`}
            ...
            <li
              className={`px-4 py-2 hover:bg-gray-700 cursor-pointer ${
                selectedLang === "np" ? "bg-gray-700 font-bold" : ""
              }`}
            ...
            <li
              className={`px-4 py-2 hover:bg-gray-700 rounded-b-2xl cursor-pointer ${
                selectedLang === "ja" ? "bg-gray-700 font-bold" : ""
              }`}
