...
export default function Profile({ userRole }) {
  ...
  const uploadImage = async (image) => {
    ...
    const response = await fetch(`${API_URL}/api/updateImage/${userId}`, {
      method: "PUT",
      body: formData,
    });
    if (response.ok) {
  ...
  const handleImageDelete = async () => {
    if (!userId) return;
    const response = await fetch(`${API_URL}/api/deleteImage/${userId}`, {
      method: "DELETE",
    });
    if (response.ok) {
  ...
  return (
    <div>
      <h1 className="text-lg  font-extrabold border-b border-gray-500">
        {t("editProfile")}
      </h1>
      <div className="grid grid-cols-1 md:grid-cols-2 md:gap-6 py-3">
        <div className="space-y-2 order-2 md:order-1">
          ...
          {userRole === "admin" && (
            <button
              className="p-2 bg-green-800 w-full rounded"
        ...
        <div className="order-1 md:order-2">
          ...
          <div className="relative">
            <img
              src={
                selectedImage || userData?.image ? `${userData?.image}` : img
              }
              alt="Profile"
            ...
            {showMessageBox && (
              <div className="absolute left-1/4 py-1 w-48 border border-gray-700 text-sm bg-gray-950 rounded-md">
                <button
                  className="block w-full text-left px-4 py-1 hover:bg-blue-800"
