...
export default function Admin() {
  ...
  const admin = async () => {
    ...
    if (
      res.status === 401 ||
      !data ||
      (data.validUser.role !== "admin" && data.validUser.role !== "staff")
    ) {
  ...
  const getPageTitle = (path) => {
    switch (path) {
      ...
      case "/admin/settings/profile":
        return t("setting");
      case "/admin/table-menu":
        return t("tableMenu");
      case "/admin/table-menu/main":
        return t("menu");
      default:
        return "Dashboard";
    }
        ...
        <div className="flex-1 overflow-y-auto bg-gray-950 ">
          <Routes>
            ...
            <Route
              path="dashboard"
              element={
                <Dashboard userRole={userRole} userCategory={userCategory} />
              }
            />
            <Route
              path="kitchen"
              element={
                <Kitchen userRole={userRole} userCategory={userCategory} />
              }
            />
            <Route path="profile" element={<Profile userRole={userRole} />} />
            <Route
              path="settings/*"
              element={<Setting userRole={userRole} />}
            />
            ...
            {userRole === "staff" && userCategory === "waiter" && (
              <>
                <Route
                  path="cashier"
                  element={<Cashier userRole={userRole} />}
                />
                <Route
                  path="table-menu"
                  element={<TableMenu userRole={userRole} />}
                />
                <Route
                  path="table-menu/main"
                  element={<Main userRole={userRole} />}
                />
              </>
            )}
